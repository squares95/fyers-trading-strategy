...
def calculate_portfolio_metrics(trades_list: list) -> dict:
    ...
    all_trades = pd.concat(
        [t["trades"] for t in trades_list if len(t["trades"]) > 0], ignore_index=True
    )
    ...
    return {
        ...
        "avg_winner_pct": (
            round(win_trades["net_return_pct"].mean(), 3) if len(win_trades) > 0 else 0
        ),
        "avg_loser_pct": (
            round(loss_trades["net_return_pct"].mean(), 3) if len(loss_trades) > 0 else 0
        ),
...
def main():
    ...
    for r in individual_results:
        print(
            f"{r['symbol']:<15} {r['trades']:>7} {r['net_return_pct']:>+8.1f} {r['win_rate_pct']:>10.1f}"
        )
    total_individual = sum(r["net_return_pct"] for r in individual_results)
    print("-" * 60)
    print(
        f"{'TOTAL (sum)':<15} {sum(r['trades'] for r in individual_results):>7} {total_individual:>+8.1f}"
    )
    ...
    print(
        f"  Profitable Months:   {portfolio_metrics['profitable_months']}/{portfolio_metrics['months_traded']}"
    )
    ...
    monthly_returns = all_trades.groupby(
        pd.to_datetime(all_trades["entry_time"]).dt.to_period("M")
    )["net_return_pct"].sum()
    ...
    print(
        f"  Sharpe-like Ratio:   {monthly_avg / monthly_std:.2f}"
        if monthly_std > 0
        else "  Sharpe-like Ratio:   N/A"
    )
