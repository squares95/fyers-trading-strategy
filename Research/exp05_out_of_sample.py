...
PORTFOLIO_STOCKS = [
    "CGPOWER", "BHEL", "DRREDDY", "INDUSINDBK",
    "M&M", "HCLTECH", "TITAN"
]
...
def calculate_metrics(trades: pd.DataFrame) -> dict:
    ...
    # Monthly
    monthly = sorted_trades.groupby(pd.to_datetime(sorted_trades['entry_time']).dt.to_period('M'))['net_return'].sum() * 100
...
def main():
    ...
    for cutoff, label in OOS_CUTOFFS:
        ...
        oos_trades = pd.concat(oos_trades_list, ignore_index=True) if oos_trades_list else pd.DataFrame()
    ...
    for period, m in results.items():
        if "error" in m:
            ...
            print(f"{period:<30} {m['trades']:>8} {m['net_return_pct']:>+8.1f} "
