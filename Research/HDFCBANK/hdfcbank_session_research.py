...
from Research.CGPOWER import premarket_context_analysis as pm
...
def prepare_context(
    ...
    for prefix, data in external.items():
        context = pd.merge_asof(
            context.sort_values("Date"), pm.make_available_next_day(data, prefix),
    ...
    context["nifty_gap_proxy"] = pd.cut(context["nifty_gap"], [-np.inf, -0.002, 0.002, np.inf],
    ...
    results = pd.concat([
        ...
        pm.summarize(context.groupby("nifty_gap_proxy", observed=True), "NIFTY opening-gap proxy"),
...
def relationship_metrics(daily: pd.DataFrame, symbol: str) -> dict[str, float | str]:
    ...
    return {
        "Symbol": symbol, "Sessions": len(daily),
        ...
        "DailyDirectionMatch": (np.sign(matched["daily_return"]) == np.sign(matched["nifty_daily_return"])).mean(),
        ...
        "BothOpeningSidesBroken": daily["break_type"].isin(["low_then_high", "high_then_low"]).mean(),
    }


def chart_context(results: pd.DataFrame) -> None:
    display = results[results["Test"].isin(["Previous US close", "US + NIFTY-gap agreement"])].copy()
...
def chart_replays(replay: pd.DataFrame, minutes: pd.DataFrame) -> None:
    ...
    for ax, (_, row) in zip(axes.flat, replay.iterrows()):
        ...
        ax.plot(np.arange(len(day)), path, color="#195B8A" if row["Direction"] > 0 else "#F28E2B", linewidth=1.4)
    ...
    fig.suptitle("HDFCBANK mental paper trades: best and worst identical-context outcomes", x=0.06, ha="left", weight="bold")
...
def main() -> None:
    ...
    for name, trades_train, trades_test in (
        ...
        ("CG_frozen_liquidity_sweep",
         sm.evaluate_sweep_rule(daily, minutes, "14:30", "midpoint", 1.0, 0.0, end=DISCOVERY_END),
         sm.evaluate_sweep_rule(daily, minutes, "14:30", "midpoint", 1.0, 0.0, start=HOLDOUT_START)),
        ("CG_frozen_ORB",
         sm.evaluate_orb_rule(daily, minutes, "11:30", "opposite_boundary", 1.5, 0.0, 0.70, False, end=DISCOVERY_END),
         sm.evaluate_orb_rule(daily, minutes, "11:30", "opposite_boundary", 1.5, 0.0, 0.70, False, start=HOLDOUT_START)),
    ):
        transferred.extend([
    ...
    cg_daily = pd.read_csv(ROOT / "Research" / "CGPOWER" / "session_daily_features.csv", parse_dates=["Date"]).set_index("Date")
...
"""
    ...
    print(f"HDFCBANK sessions: {len(daily)} ({daily.index.min().date()}..{daily.index.max().date()})")
