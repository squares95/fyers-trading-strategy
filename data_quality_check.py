...
def check_symbol_quality(symbol):
    ...
    outside_regular = df[
        (df["time"] < market_open) | (df["time"] > pd.Timestamp("15:29:00").time())
    ]
...
for sym, r in results.items():
    print(
        f"{sym:10} | rows:{r['rows']:8,} | dup:{r['dup']:4} | gaps>1:{r['gaps_gt1']:4} | ohlc_bad:{r['ohlc_bad']:6} | zero_vol:{r['zero_vol']:6} | outside:{r['outside_regular']:4}"
    )
