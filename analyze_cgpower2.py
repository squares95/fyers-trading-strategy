...
if len(intra_day_gaps) > 0:
    print(
        intra_day_gaps[["Datetime", "minute_diff", "Open", "High", "Low", "Close", "Volume"]]
        .head(20)
        .to_string()
    )
...
if len(zero_vol) > 0:
    ...
    print("   Time distribution (top 15):")
...
if (buffer["Volume"] > 0).sum() > 0:
    ...
    print(
        buffer[buffer["Volume"] > 0][["Datetime", "Open", "High", "Low", "Close", "Volume"]]
        .head(10)
        .to_string()
    )
